...
import com.laka.libutils.StringUtils
...
class PagerListTabView : View {
    ...
    //属性动画
    private val animator: ValueAnimator = ValueAnimator.ofFloat(0f, 1f).setDuration(300)
    //位置
    ...
    private fun onDrawText(canvas: Canvas?) {
        canvas?.let {
            ...
            drawText(it, ContextCompat.getColor(context, R.color.white),
                    mSliderLeft, mSliderLeft + mSliderRange,
                    leftTextX, leftTextY,
                    mLeftTabStr)
            drawText(it, ContextCompat.getColor(context, R.color.blue),
    ...
    private fun drawText(canvas: Canvas, @ColorInt color: Int,
